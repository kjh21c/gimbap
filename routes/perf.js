...
var metar_data 
	...
	,Perf172_data_TO_Gnd_Roll
	,Perf172_data_TO_50_Clr
	,Perf172_data_Land_Gnd_Roll
	,Perf172_data_Rate_Of_Climb;
...
var readData = function(callback) {
	...
	var url = { 
			url : 
				"http://aviationweather.gov/adds/dataserver_current/httpparam?dataSource=metars&requestType=retrieve&format=xml&stationString=KHIO&hoursBeforeNow=2"
			,json: false
		};
		...
		request(url, function (err, res, html) {
			...
			//		console.log('crossWind2:'+crossWind2);
				...
				//data 152		
				var takeoffGndRoll_152 = [[640,695,755,810] // p. alti 0 temp 0~30
				...
				//data 172		
				var takeoffGndRoll_172 = 
					[
						[	795	,	860	,	925	,	995	]
						,[	875	,	940	,	1015	,	1090	]
						,[	960	,	1035	,	1115	,	1200	]
						];			// p. alti 2000 temp 0~30
				...
				//
				// 데이터 저장 152
				Perf152_data_TO_Gnd_Roll = CurTOGndRoll(metar_temp,pa,takeoffGndRoll_152).toFixed(1);
				Perf152_data_TO_50_Clr = CurTO50Clr(metar_temp,pa,takeoff50ft_152).toFixed(1);
				Perf152_data_Land_Gnd_Roll = CurLandRoll(metar_temp,pa,landGndRoll_152).toFixed(1);
				Perf152_data_Rate_Of_Climb = CurRateOfClimb(metar_temp,pa,rateOfClimb_152).toFixed(1);
				
				// 데이터 저장 172
				Perf172_data_TO_Gnd_Roll = CurTOGndRoll(metar_temp,pa,takeoffGndRoll_172).toFixed(1);
				Perf172_data_TO_50_Clr = CurTO50Clr(metar_temp,pa,takeoff50ft_172).toFixed(1);
				Perf172_data_Land_Gnd_Roll = CurLandRoll(metar_temp,pa,landGndRoll_172).toFixed(1);
				Perf172_data_Rate_Of_Climb = CurRateOfClimb(metar_temp,pa,rateOfClimb_172).toFixed(1);
				...
				function Forecast(x, kx, ky){
				  ...
				  return (a+b*x);
					...
					function CurTOGndRoll(cur_temp,cur_pressure, takeoffGndRoll) //temp , pre , 데이터 
					{
						//T.O Gnd Roll
						var ToGndRollForecast = new Array;
						for(i=0;i<takeoffGndRoll.length;i++){
							ToGndRollForecast[i] = Forecast(cur_temp, tempDegreeby10 ,takeoffGndRoll[i]); // 0 alt. pressure 0 ,1000,2000일때
						}
						var curToGndRoll =
					...
					function CurTO50Clr(cur_temp,cur_pressure, takeoff50ft){ 
						...
						for(i=0;i<takeoff50ft.length;i++){
							To50ftForecast[i]  = Forecast(cur_temp, tempDegreeby10 ,takeoff50ft[i]);
						}
						var curTo50ft =
					...
					function CurLandRoll(cur_temp,cur_pressure, landGndRoll){ 
						...
						for(i=0;i<landGndRoll.length;i++){
							LandGndRollForecast[i] =   Forecast(cur_temp, tempDegreeby10 ,landGndRoll[i]);
						}
						var curLandGndRoll =
					...
					function CurRateOfClimb(cur_temp,cur_pressure, rateOfClimb){ 
						...
						for(i=0 ;i<rateOfClimb.length;i++){
							rateOfClimbForecast[i] = Forecast(cur_temp, tempDegreeby20 ,rateOfClimb[i]) 
						}
						var curRateOfClimb =
